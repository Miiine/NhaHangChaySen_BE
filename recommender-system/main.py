...
# Lấy tham số 'maTaiKhoan' từ dòng lệnh
...
favorite_dishes_df = pd.read_sql(query_favorite_dishes, engine)
ratings_df = pd.read_sql(query_ratings, engine)
search_history_df = pd.read_sql(query_search_history, engine)
mon_an_names_df = pd.read_sql(query_dishes_names, engine)
...
user_data_result_dict = user_data_sorted_with_names.to_dict(orient='records')
json_data = json.dumps(user_data_result_dict, ensure_ascii=False)
